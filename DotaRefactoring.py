...
for x in range(len(img_input)):
    ...
    #
    # check if harbour is a label in the text_file
    #
    harbor_check_array = []
    harbor_list = []
    ...
    harbor = "harbor"
    ship = "ship"
    harbor_in_list = any(harbor in string for string in harbor_list)
    ship_in_list = any(ship in string for string in harbor_list)
    if not harbor_in_list and ship_in_list == True:
        #
        # check for the ship label
        #
        ...
        for i in range(len(txt_file)):
            ...
            if single_label == "ship":
                single_x_array = np.array([single_x1, single_x2, single_x3, single_x4]).astype(float)
                single_xmax = single_x_array.max()
                x_max_list.append(single_xmax)
                single_xmin = single_x_array.min()
                x_min_list.append(single_xmin)
                single_y_array = np.array([single_y1, single_y2, single_y3, single_y4]).astype(float)
        ...
        if len(x_max_list) > 0:
            for line in txt_file:
                ...
                if (label_name == "ship"):
                    ...
                    xarray = np.array([x1, x2, x3, x4]).astype(float)
                    xmax = xarray.max()
                    xmin = xarray.min()
                    # create array with y values and get min/max
                    yarray = np.array([y1, y2, y3, y4]).astype(float)
                    ymax = yarray.max()
                    ymin = yarray.min()
                    ...
                    concat = str(label_number) + " " + str(xczeroed) + " " + str(yczeroed) + " " + str(
                        xdistance) + " " + str(ydistance)
                    export_list.append(concat)
                    # check if other ships are fully visible in the cropped image
                    return_array = calcExportList(cropped_coordinates, x_max_list, x_min_list, y_max_list, y_min_list,
                                                  export_list, pos_in_list)
                    ...
                    # remove duplicates from export_list
                    export_list = set(export_list)
